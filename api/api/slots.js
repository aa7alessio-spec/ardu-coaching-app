// Liste / création / suppression de créneaux
// Stockage : Vercel KV (Upstash) via REST (aucune lib à installer)
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const slots = await kvGet('slots') || [];
      const slot = { id: uid(), theme: theme || 'Séance Ardu Coaching', type: type || 'individuel', datetime, capacity: Number(capacity), booked: 0 };
      slots.push(slot);
      await kvSet('slots', slots);

      // SMS aux clientes (si numéros fournis)
      if (TW_SID && TW_TOKEN && TW_FROM && CLIENT_NUMBERS.length && broadcastMessage) {
        const client = twilio(TW_SID, TW_TOKEN);
        await Promise.allSettled(CLIENT_NUMBERS.map(n =>
          client.messages.create({ from: TW_FROM, to: n, body: broadcastMessage })
        ));
      }
